    ...
    public function bulkBook(Request $request)
    {
        ...
        try {
            while (($line = fgetcsv($file)) !== false) {
                $weekday = date('l', strtotime($line[2]));
                $time = $line[3];


                $user = User::where('name', $line[0])->firstOrFail();

                $availability = DentistAvailability::where('dentist_id', $line[1])
                    ->where('days', $weekday)
                    ->whereTime('time', $time)
                    ->first();
                ...
                if ($availability) {
                    ...
                    $time = DentistAvailability::where('availability_id', $availability->availability_id)->firstOrFail();
                    $time->isBooked = 1;
                    $time->save();
                    ...
                    $formatTime = Carbon::createFromFormat('H:i:s', $line[3])->format('h:i A');
                    $formatDate = Carbon::createFromFormat('Y-m-d', $line[2])->format('F d, Y');
