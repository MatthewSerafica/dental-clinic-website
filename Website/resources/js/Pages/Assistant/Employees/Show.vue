            ...
            <div class="relative flex flex-col">
                <div class="absolute right-0 w-80">
                    <Toast x-data="{shown: false, timeout:null}"
                        ...
                        x-show.transition.opacity.out.duration.5000ms="shown" class=""
                        v-if="flashSuccess && isShowFlash" closable type="success">
                        {{ flashSuccess }}
                    </Toast>
                    <Toast x-data="{shown: false, timeout:null}"
                        x-init="clearTimeout(timeout); shown = true; timeout = setTimeout(() => { shown = false }, 5000);"
                        x-show.transition.opacity.out.duration.5000ms="shown" class=""
                        v-if="flashError && isShowFlash" closable type="danger">
                        {{ flashError }}
                    </Toast>
                </div>
