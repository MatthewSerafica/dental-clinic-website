    ...
    <form @submit.prevent="update">
        <Modal size="4xl" class="md:flex md:space-y-0 md:space-x-4" @close="closeModal">
            ...
            <template #body>
                ...
                <div class="px-4">
                    <!---start of row 1-->
                    <div class="w-64 max-md:w-full pb-2">
                    ...
                    <div class="flex flex-row gap-4 pb-2 max-md:flex-col max-md:pb-4">
                       <div class="max-md:w-full">
                            <label for="school" class="block font-semibold">School <label
                                    class=" text-[red]">*</label></label>
                            <select class="h-10 rounded-md drop-shadow-lg max-md:w-full" v-model="form.school" @change="fetchCourses"
                                required>
                    ...
                    <div class="flex flex-row gap-4 pb-2 max-md:flex-col">
                        ...
                        <div class="">
                            ...
                            <input id="birth_date" v-model="form.birth_date" type="date"
                                class="h-10 rounded-md drop-shadow-lg w-[9em] max-sm:w-full " placeholder="Select Date"
                                required>
                        </div>
                        ...
                        <div class="w-52 max-md:w-full">
                            ...
                            <label class="block font-semibold">
                                <input name="cardio_option" id="no" type="radio" class=""
                                    v-model="form.cardiovascular_disease_option" value="NO">
                                NO
                            </label>
                            <label class="block font-semibold">
...
const fetchCourses = async () => {
    try {
        ...
        if (courseId) {
            const response = await axios.get(route('admin.student.course.index', courseId))
...
const update = () => form.put(route('admin.student.update', props.student.id_number), { preserveScroll: false, preserveState: false })
</script>