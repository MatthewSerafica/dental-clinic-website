                    ...
                    <div class="flex flex-row gap-12 max-md:flex-col">
                        ...
                        <div class="w-[25rem]">
                            <div class="w-[25rem] mb-4">
                                ...
                                <input id="patientId" v-model="idNumber" type="text" disabled
                            ...
                            <div class="mb-4 w-[25rem]">
                                ...
                                <select id="appointmentType" v-model="form.app_type" disabled
                                    class="border border-black w-[25rem] px-3 py-2 rounded-md drop-shadow-lg">
                        ...
                        <div class="">
                            ...
                            <div class="grid grid-cols-5 gap-4 mb-4">
                                <div v-for="available in availableTime.morningTime">
                                    <Button v-if="!available.isBooked"
                                        @click="setTime(available.time, available.availability_id)" type="button"
                                        as="button"
                                    ...
                                    <Button v-else @click="setTime(available.time, available.availability_id)" type="button"
                            ...
                            <div class="grid grid-cols-5 gap-4">
                                <div v-for="available in availableTime.eveningTime">
                                    <Button v-if="!available.isBooked"
                                        @click="setTime(available.time, available.availability_id)" type="button"
...
onMounted(async () => {
    if (props.appointment.user.user_type === "student") {
        idNumber.value = props.appointment.user.student.id_number;
        last_name.value = props.appointment.user.student.last_name + ", ";
        first_name.value = props.appointment.user.student.first_name;
        middle_name.value = props.appointment.user.student.middle_name;
        ...
        if ((props.appointment.user.student.course == 'Doctor of Medicine' || props.appointment.user.student.course == 'BSN') && props.appointment.user.student.year != '1st Year') {
            const servicesResponse = await axios.get(`/assistant/service/annual`);
            services = servicesResponse.data;
        } else {
            const servicesResponse = await axios.get(`/assistant/service/regular`);
            services = servicesResponse.data;
        }
    } else {
...
const fetchData = async () => {
    if (idNumber.value.length === 7 && idNumber.value) {
        try {
            if (idNumber.value.includes(' ')) {
                const response = await axios.get(route('assistant.appointment.showEmployee', idNumber.value));
                ...
                fullName.value = employees.value.last_name + ", " + employees.value.first_name + ", " + employees.value.middle_name;
                form.name = employees.value.last_name + ", " + employees.value.first_name;

                const servicesResponse = await axios.get(route('assistant.getemployeeservice'));
                services = servicesResponse.data;

            } else {
                const response = await axios.get(route('assistant.appointment.showStudent', idNumber.value));
                ...
                fullName.value = students.value.last_name + ", " + students.value.first_name + ", " + students.value.middle_name;
                ...
                if ((studentData.course == 'Doctor of Medicine' || studentData.course == 'BSN') && studentData.year != '1st Year') {
                    const servicesResponse = await axios.get(route('assistant.getannualservices'));
                    services = servicesResponse.data;
                } else {
                    const servicesResponse = await axios.get(route('assistant.getregularservices'));
                    services = servicesResponse.data;
                }
            }
...
const fetchAvailableTime = async (id, date) => {
    try {
        const response = await axios.get(route('assistant.appointment.getAvailableTime', { id, date }))
...
const form = useForm({
    ...
    app_date: null,
    app_time: null,
    app_type: props.appointment.app_type,
    pat_notes: props.appointment.pat_notes,
    time_id: null,
    old_time_id: null,
})
...
const setTime = (time, id) => {
    ...
    form.time_id = id
    form.old_time_id = props.appointment.availability_id
}
...
const update = () => form.put(route('assistant.appointment.edit', props.appointment.app_id), { preserveScroll: false, preserveState: false })
