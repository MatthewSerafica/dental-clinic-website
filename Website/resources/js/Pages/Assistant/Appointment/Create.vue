<template>
    <form @submit.prevent="create">
        <Modal size="5xl" @close="closeModal">
            <template #header>
            ...
            <template #body>
                <div class="p-2">
                    <div class="flex flex-row gap-12">
                        ...
                        <!-- Right Column -->
                        <div class="">
                            <h2 class="mb-2 text-lg font-semibold text-center">Morning Session</h2>
                            <div class="grid grid-cols-4 gap-4 mb-4">
                                <div v-for="available in availableTime.morningTime">
                                    <Button v-if="!available.isBooked"
                                        @click="setTime(available.time, available.availability_id)"
                                        type="button" as="button" class="px-4 py-4 border-2 " outline>
                                        <p class="text-base">
                                    ...
                                    <Button v-else
                                        @click="setTime(available.time, available.availability_id)"
                                        type="button" as="button" class="px-4 py-4 border-2 border-red-600" color="red"
                                        disabled>
                                        <p class="text-base">
                                            {{ formatTime(available.time) }}
                                        </p>
                            ...
                            <h2 class="mb-2 text-lg font-semibold text-center">Afternoon Session</h2>
    ...
    if (idNumber.value.length === 7) {
        try {
            if (idNumber.value.includes(' ')) {
                const response = await axios.get(`/assistant/appointment/employee/${idNumber.value}`);
                ...
                const response = await axios.get(`/assistant/appointment/student/${idNumber.value}`);
...
const create = () => form.post(route('assistant.appointment.store'), { preserveScroll: false, preserveState: false })
</script>
