...
onMounted(async () => {
    if (props.appointment.user.user_type === "student") {
        ...
        if ((props.appointment.user.student.course == 'Doctor of Medicine' || props.appointment.user.student.course == 'BSN') && props.appointment.user.student.year != '1st Year') {
            const servicesResponse = await axios.get(`/patient/service/annual`);
            services = servicesResponse.data;
        } else {
            const servicesResponse = await axios.get(`/patient/service/regular`);
            services = servicesResponse.data;
        }
    } else {
    ...
    if (idNumber.value.length === 7 && idNumber.value) {
        try {
            if (idNumber.value.includes(' ')) {
                const response = await axios.get(route('patient.appointments.showEmployee', idNumber.value));
                ...
                const response = await axios.get(route('patient.appointment.showStudent', idNumber.value));
...
const update = () => form.put(route('patient.appointments.edit', props.appointment.app_id), { preserveScroll: false, preserveState: false })
