    ...
    <form @submit.prevent="update">
        <Modal size="4xl" class="md:flex md:space-y-0 md:space-x-4" @close="closeModal">
            ...
            <template #body>
                ...
                <div class="px-4">
                     <!---start of row 1-->
                    <div class="w-64 max-sm:w-full pb-1">
                    ...
                    <div class="flex flex-row gap-4 pb-2 max-sm:flex-col">
                        <div class="max-sm:w-full">
                            <label for="school" class="block font-semibold">School <label
                                    class=" text-[red]">*</label></label>
                            <select class="h-10 rounded-md drop-shadow-lg max-sm:w-full" v-model="form.school" @change="fetchCourses"
                                required>
                    ...
                    <div class="flex flex-row gap-4 pb-2 max-sm:flex-col">
                        ...
                        <div class="">
                            ...
                            <input id="birth_date" v-model="form.birth_date" type="date"
                                class="h-10 rounded-md drop-shadow-lg w-[9em] max-sm:w-full"
                                placeholder="Select Appointment Date" required>
                            <div v-if="form.errors.birth_date" class="text-red-600">
...
const fetchCourses = async () => {
    try {
        ...
        if (courseId) {
            const response = await axios.get(route('student.course.index', courseId))
...
watch(() => form.id_number, async (newValue) => {
    form.errors.id_number = !newValue
        ...
        : !/^[2]\d+$/.test(newValue)
            ? 'ID number should start with 2'
            : newValue < 1000000 || newValue > 9999999
                ? 'ID number should have 7 digits'
                : /^\D+$/.test(newValue)
    ...
    if (!form.errors.id_number) {
        try {
            const { data } = await axios.get(route('student.check', newValue));
            if (data.exists) {
...
const update = () => form.put(route('student.update', props.student.id_number), { preserveScroll: false, preserveState: false })
</script>