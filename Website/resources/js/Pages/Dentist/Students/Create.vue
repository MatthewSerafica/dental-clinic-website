                ...
                <div class="px-4">
                    ...
                    <div class="w-64 max-sm:w-full pb-1">
                            ...
                            <input @keydown.space.prevent id="patientId" v-model.number="form.id_number" type="text"
                                class="w-[15.7em] h-10 rounded-md drop-shadow-lg max-sm:w-full"
...
watch(() => form.id_number, async (newValue) => {
    form.errors.id_number = !newValue
        ...
        : !/^[2]\d+$/.test(newValue)
            ? 'ID number must start with 2'
            : newValue < 1000000 || newValue > 9999999
                ? 'ID number must have 7 digits'
                : /^\D+$/.test(newValue)
...
const create = () => { form.post(route("student.store"), { preserveScroll: false, preserveState: false }) }
