...
watch(() => form.id_number, async (newValue) => {
    ...
    if (!form.errors.id_number) {
        try {
            const { data } = await axios.get(route('employee.check', newValue));
            if (data.exists) {
...
const update = () => form.put(route('employee.update', props.employee.id_number), { preserveScroll: false, preserveState: false })
</script>