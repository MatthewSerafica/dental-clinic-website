    ...
    if (idNumber.value.length === 7 && idNumber.value) {
        try {
            if (idNumber.value.includes(' ')) {
                const response = await axios.get(route('appointment.showEmployee', idNumber.value));
                ...
                const response = await axios.get(route('appointment.showStudent', idNumber.value));
...
const update = () => form.put(route('appointment.edit', props.appointment.app_id), { preserveScroll: false, preserveState: false })
