  ...
  <MainLayout>
    <template v-slot:mainContent>
      <div class="absolute right-4 w-80">
        ...
        <Toast x-data="{shown: false, timeout:null}"
          x-init="clearTimeout(timeout); shown = true; timeout = setTimeout(() => { shown = false }, 5000);"
          x-show.transition.opacity.out.duration.5000ms="shown" v-if="flashError && isShowFlash" closable type="danger">
          {{ flashError }}
        </Toast>
      </div>
      <div class="flex flex-col ml-2">
        <div class="flex flex-row align-items-center">
          ...
          <div class="">
            <button @click="showModal" type="button" class="z-50 w-10 h-8 bg-blue-500 rounded-lg hover:bg-blue-700" v-if="appointment.status == 'approved'">
...
import Edit from '@/Pages/Dentist/Appointment/Edit.vue';
