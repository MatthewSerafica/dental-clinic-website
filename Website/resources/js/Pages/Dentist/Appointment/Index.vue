              ...
              <table class="w-full my-5 overflow-hidden rounded-lg sm:bg-white sm:shadow-lg">
                <thead class="w-full text-black">
                  <tr
                    ...
                    <th class="p-3 text-left max-md:hidden">ACTIONS</th>
                  </tr>
                </thead>
                  ...
                  <tr
                    ...
                    <div class="lg:hidden">
                      <td class="p-3 border-grey-light hover:bg-gray-100 ">
                        <Link :href="`/appointment/${appointment.app_id}`">
...
import Cancel from '@/Pages/Dentist/Appointment/Cancel.vue';
import Create from "@/Pages/Dentist/Appointment/Create.vue";
...
function markAsPending(appointment) {
  pending.put(route("appointment.update", appointment.app_id));
}
function markAsApproved(appointment) {
  approve.put(route("appointment.update", appointment.app_id));
}
function markAsOngoing(appointment) {
  ongoing.put(route("appointment.update", appointment.app_id));
}
function markAsCompleted(appointment) {
  complete.put(route("appointment.update", appointment.app_id));
}
