    ...
    <MainLayout>
        <template v-slot:mainContent>
            <div class="flex flex-col ml-2">
                <h1 class="w-20 h-8 text-2xl font-bold text-gray-700">Services</h1>
                <h6 class="h-1 text-xs text-gray-400">
                    <a href="/admin">Home</a> > Services
                </h6>
            </div>
            ...
            <div class="flex flex-col flex-grow mt-8 bg-white rounded-xl">
                            ...
                            <table class="w-full my-5 overflow-hidden rounded-lg sm:bg-white sm:shadow-lg">
                                ...
                                <tbody class="flex-1 sm:flex-none">
                                    <tr class="flex flex-col mb-2 flex-no wrap sm:table-row sm:mb-0 max-sm:border max-sm:roun hover:bg-gray-100"
                                        v-for="(service, index) in services.data" :key="service.id">
                                        <td
                                        ...
                                        <td class="p-3 border-grey-light sm:border-b hover:bg-gray-100 max-sm:hidden w-52">
                                            <p class="mr-2 text-base font-medium leading-none text-gray-700">
                                                {{ service.regular ? 'Yes' : 'No' }}
                                            </p>
                                        </td>
                                        <td class="p-3 border-grey-light sm:border-b hover:bg-gray-100 max-sm:hidden w-52">
                                            <p class="mr-2 text-base font-medium leading-none text-gray-700">
                                                {{ service.annual ? 'Yes' : 'No' }}
                                            </p>
                                        </td>
                                        <td class="p-3 border-grey-light sm:border-b hover:bg-gray-100 max-sm:hidden w-52">
                                            <p class="mr-2 text-base font-medium leading-none text-gray-700">
                                                {{ service.employee ? 'Yes' : 'No' }}
                                            </p>
                                        </td>
                                        <td class="p-3 border-grey-light sm:border-b hover:bg-gray-100 max-sm:hidden w-52">
                                            <p class="mr-2 text-base font-medium leading-none text-gray-700">
                                                {{ formatDate(new Date(service.created_at).toLocaleDateString()) }}
                                            </p>
                                        </td>
                                        <td class="w-2 p-3 border-grey-light sm:border-b hover:bg-gray-100 max-sm:hidden">
                                            <div class="flex flex-row gap-2">
                                                <button @click="showEdit(service)" type="button"
                                                    class="px-4 py-2 text-gray-100 bg-blue-500 rounded-lg max-sm:w-full hover:bg-blue-700 hover:text-white">
                                                    Edit
                                                </button>
                                                <button @click="showDelete(service)" type="button"
                ...
                <div v-if="services.data.length" class="flex justify-center w-full mb-5">
                    <Pagination :links="services.links" />
                </div>

                <Create v-if="isShowModal" @closeModal="closeModal" />
                <Edit v-if="isShowEdit" :service="selectedService" @closeEdit="closeEdit" />
                <Delete v-if="isShowDelete" :service="selectedService" @closeDelete="closeDelete" />
...
import Create from "@/Pages/Admin/Services/Create.vue";
import Delete from "@/Pages/Admin/Services/Delete.vue";
import Edit from "@/Pages/Admin/Services/Edit.vue";
...
function showEdit(department) {
    if (!isShowEdit.value) {
        selectedService.value = department
