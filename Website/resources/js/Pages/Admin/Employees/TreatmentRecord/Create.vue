...
const createRecord = () => form.post(
    route("admin.employee.treatment.store", props.employee.id_number), { preserveScroll: false, preserveState: false }
)
</script>
      ...
      <Modal size="3xl" @close="closeModal">
          ...
          <div class="p-4">
            ...
            <div class="grid grid-cols-3 sm:grid-cols-3">
              ...
              <label for="time_in" class="block font-semibold">Time In</label>
                    <input id="time_in" v-model="form.time_in" type="time" step="60" class="w-[10rem] px-3 py-2 border rounded" required>   
              </div>
  
 
              <div class="mb-4">
              <label for="time_out" class="block font-semibold">Time Out</label>
                    <input id="time_out" v-model="form.time_out" type="time" step="60" class="w-[10rem] px-3 py-2 border rounded" required>   
              </div>
            </div>
            ...
            <div class="grid grid-cols-1 left-10 gap-y-4">
              <div class="mb-4">
                ...
                <select class="w-[12.9rem] px-3 py-2 border rounded" v-model="form.service_rendered">
                  <option disabled>Please select one</option>
                  <option>Consultation</option>
                  <option>Extraction</option>
                  <option>Annual Check-up</option>
                  <option>Emergency Cases</option>
        ...
        <template #footer>
          <div class="flex justify-center">
            ...
            <button type="submit" as="button" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800 mr-3 mb-3">
              Add Record
            </button>
          </div>
        </template>
