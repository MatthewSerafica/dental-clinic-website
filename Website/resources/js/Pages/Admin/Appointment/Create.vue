    ...
    if (idNumber.value.length === 7) {
        try {
            if (idNumber.value.includes(' ')) {
                const response = await axios.get(`/admin/appointment/employee/${idNumber.value}`);
                ...
                const servicesResponse = await axios.get(route('admin.employee.getemployeeservice'));
                services = servicesResponse.data;
            } else {
                const response = await axios.get(`/admin/appointment/student/${idNumber.value}`);
                ...
                if ((studentData.course == 'Doctor of Medicine' || studentData.course == 'BSN') && studentData.year != '1st Year') {
                    const servicesResponse = await axios.get(route('admin.student.getannualservices'));
                    services = servicesResponse.data;
                } else {
                    const servicesResponse = await axios.get(route('admin.student.getregularservices'));
                    services = servicesResponse.data;
                }
            }
...
const create = () => form.post(route('admin.appointment.store'), { preserveScroll: false, preserveState: false })
</script>
