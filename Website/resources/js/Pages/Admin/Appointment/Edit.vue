...
onMounted(async () => {
    if (props.appointment.user.user_type === "student") {
        ...
        if ((props.appointment.user.student.course == 'Doctor of Medicine' || props.appointment.user.student.course == 'BSN') && props.appointment.user.student.year != '1st Year') {
            const servicesResponse = await axios.get(`/admin/services/annual`);
            services = servicesResponse.data;
        } else {
            const servicesResponse = await axios.get(`/admin/services/regular`);
            services = servicesResponse.data;
        }
    } else {
    ...
    if (idNumber.value.length === 7 && idNumber.value) {
        try {
            if (idNumber.value.includes(' ')) {
                const response = await axios.get(route('admin.appointment.showEmployee', idNumber.value));
                ...
                const servicesResponse = await axios.get(route('admin.getemployeeservice'));
                services = servicesResponse.data;

            } else {
                const response = await axios.get(route('admin.appointment.showStudent', idNumber.value));
                ...
                if ((studentData.course == 'Doctor of Medicine' || studentData.course == 'BSN') && studentData.year != '1st Year') {
                    const servicesResponse = await axios.get(route('admin.getannualservices'));
                    services = servicesResponse.data;
                } else {
                    const servicesResponse = await axios.get(route('admin.getregularservices'));
                    services = servicesResponse.data;
                }
            }
...
const fetchAvailableTime = async (id, date) => {
    try {
        const response = await axios.get(route('admin.appointment.getAvailableTime', { id, date }))
...
const update = () => form.put(route('admin.appointment.edit', props.appointment.app_id), { preserveScroll: false, preserveState: false })
