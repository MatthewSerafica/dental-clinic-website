<template>
    <div class="flex flex-col items-center">
        ...
        <div class="relative top-[10px] flex flex-col gap-10 items-center">
            <div>
                <h2 class="text-lg font-medium text-black title-font">MONTHLY OUTPUT SUMMARY</h2>
                <h2 class="text-lg font-medium text-black px-9 title-font">for DENTAL SERVICES</h2>
            </div>
        </div>
        <div class="ml-auto">
            <h2 class="pt-5 text-lg font-medium text-black items-lef title-font">Month & Year: <label class="underline">{{
                monthName }} {{ currentYear }}</label></h2>
        </div>
    </div>
    <div>
        <div>
            ...
            <table class="border border-spacing-2 border-slate-700">
                <thead>
                    <tr>
                        ...
                        <th class="border border-slate-700 w-[8em]" v-for="school in schools" :key="school"><i>{{ school }}</i>
                        </th>
                ...
                <tbody>
                    <tr class="border border-slate-700 inline-center" v-for="appType in appTypes" :key="appType">
                        ...
                        <td class="text-center">{{ getTotalForAppType(appType) }}</td>
                    </tr>
                </tbody>
            </table>
        </div>
        <div class="relative flex flex-col float-right">
            <table class="w-[15.75em] border border-slate-700">
            ...
            <table class="w-3/4 border border-spacing-2 border-slate-700 mt-[0.5em] w-[32em]">
                <thead>
                    <tr>
                        ...
                        <th v-for="year in years" :key="year" class="border border-slate-700">{{ year }}</th>
                ...
                <tbody>
                    <tr v-for="course in courses" :key="course">
                        ...
                        <td v-for="year in years" :key="year" class="text-center border border-slate-700 w-[6em]">{{
                            annualTable[course][year] || 0 }}</td>
                        <td class="text-center border border-slate-700 w-[6em]">{{ getTotalForYear(course) }}</td>
                    </tr>
                </tbody>
            </table>
        </div>
        ...
        <div>
            ...
            <table class="w-3/4 border border-spacing-2 border-slate-700 mt-[0.5em] w-[19em]">
...
const props = defineProps({
    ...
    annualStudentSummary: Object,
    employeeSummary: Object,
})

const frequencyTable = ref({})

frequencyTable.value = props.monthlySummary
...
annualTable.value = props.annualStudentSummary
...
employeeTable.value = props.employeeSummary
