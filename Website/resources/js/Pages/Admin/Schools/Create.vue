        ...
        <Modal size="xl" class="block space-y-4 md:flex md:space-y-0 md:space-x-4" @close="closeModal">
            ...
            <template #body>
                <div class="w-full mb-4 max-sm:m-0">
                    ...
                    <input @keydown.space.prevent id="department" v-model="createForm.school" type="text"
                        class="w-full px-3 py-2 rounded-md drop-shadow-lg"
                        :class="{ 'border-red-600': createForm.errors.school }" placeholder="Enter school name..."
                        required>
                    <div v-if="createForm.errors.school" class="px-2 text-sm text-red-600">
...
const create = () => { createForm.post(route('admin.schools.store'), { preserveScroll: false, preserveState: false }) }
</script>