    ...
    <MainLayout>
        <template v-slot:mainContent>
            <div class="flex flex-col ml-2 max-sm:px-5">
                <h1 class="w-20 h-8 text-2xl font-bold text-gray-700 max-sm:ml-4">Schools</h1>
                <h6 class="h-1 text-xs text-gray-400 max-sm:ml-4">
                    <a href="/admin">Home</a> > Schools
                </h6>
            </div>
            ...
            <div class="relative flex flex-col flex-grow mt-8 bg-white max-sm:ml-10 rounded-xl">
                ...
                <div class="grid mb-10 ml-10 mr-10">
                            ...
                            <table class="w-full my-5 overflow-hidden rounded-lg sm:bg-white sm:shadow-lg">
                                ...
                                <tbody class="flex-1 sm:flex-none">
                                    <tr class="flex flex-col mb-2 flex-no wrap sm:table-row sm:mb-0 max-sm:border max-sm:roun hover:bg-gray-100"
                                        v-for="(school, index) in schools.data" :key="school.id">
                                        <td
                    ...
                    <div v-if="schools.data.length" class="flex justify-center w-full mt-6">
                        <Pagination :links="schools.links" />
                    </div>
                </div>

                <Create v-if="isShowModal" @closeModal="closeModal" />
                <Edit v-if="isShowEdit" :school="selectedSchool" @closeEdit="closeEdit" />
                <Delete v-if="isShowDelete" :school="selectedSchool" @closeDelete="closeDelete" />
...
import Create from "@/Pages/Admin/Schools/Create.vue";
import Delete from "@/Pages/Admin/Schools/Delete.vue";
import Edit from "@/Pages/Admin/Schools/Edit.vue";
...
function showEdit(department) {
    if (!isShowEdit.value) {
        selectedSchool.value = department
