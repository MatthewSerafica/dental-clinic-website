        ...
        <Modal size="xl" class="block space-y-4 md:flex md:space-y-0 md:space-x-4" @close="closeEdit">
            ...
            <template #body>
                <div class="w-full mb-4 max-sm:m-0">
                    <label for="department" class="block font-semibold">School Name
                        <label class=" text-[red]">*</label>
                    </label>
                    <input id="department" v-model="editForm.school" type="text"
                        class="w-full px-3 py-2 rounded-md drop-shadow-lg"
                        :class="{ 'border-red-600': editForm.errors.school }" placeholder="Enter school name..."
                        required>
                    <div v-if="editForm.errors.school" class="px-2 text-sm text-red-600">
...
const editForm = useForm({
    school: props.school.school ? props.school.school : '',
    id: props.school.id
})


const update = () => { editForm.put(route('admin.schools.update'), { preserveScroll: false, preserveState: false }) }
</script>